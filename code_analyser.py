...
def analyzer_code_context(query: str) -> str:
...
def create_prompt_node(state:CodeState) -> CodeState:
    ...
    query = state["query"]
    context = analyzer_code_context(query)
...
st.title("Code Analyzer Chatbot(Langraph + LLM)")
...
if user_code:
    try:
        with st.spinner("Analyzing code . . ."):
            initial_state = {"query": user_code,
            "context": "", "messages": [], "response": ""}
            result = code_workflow.invoke(initial_state)
            bot_response_text = result.get("response", "No response genreated")
